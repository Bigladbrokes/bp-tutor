...
| `students`  | students + teacher | `studentName`, `studentEmail`, `photoURL`, `role` (`teacher` / `student`), `tokenBalance` (running total), `completedSessions` (map of finished session ids — a finished session shows the done screen on every device), `createdAt` |
